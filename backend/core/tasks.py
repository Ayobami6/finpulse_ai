...
def run_clustering_task():
    ...
    for label, cluster_chats in clusters_data.items():
        ...
        sentiments = [c.sentiment_score for c in cluster_chats if c.sentiment_score is not None]
        cluster.sentiment_score = sum(sentiments) / len(sentiments) if sentiments else 0.0
        ...
        if not hasattr(cluster, 'actions') or not cluster.actions.exists():
            actions_data = AIService.generate_actions(cluster.description, cluster.root_cause_analysis)
            if actions_data:
